// ============================================================
//  StructureEditorPanel — UE-style Structure Definition Editor
//  Opens in a dockview panel. Allows editing fields: name, type,
//  default value, tooltip. Supports add/remove/reorder.
// ============================================================
...
export class StructureEditorPanel {
  ...
  private _emit(): void {
    this._manager.notifyStructureChanged(this._asset.id);
    this._onChanged?.();
  }
  ...
  private _buildTypeOptions(selected?: VarType): string {
    ...
    // Project-level structs (excluding self to prevent circular)
    for (const s of this._manager.structures) {
      if (s.id === this._asset.id) continue; // no self-reference
      const val: VarType = `Struct:${s.id}`;
      html += `<option value="${val}"${selected === val ? ' selected' : ''}>${s.name} (Struct)</option>`;
    }
    // Project-level enums
    for (const e of this._manager.enums) {
  ...
  private _build(): void {
    ...
    // Category
    const catRow = this._createPropertyRow('Category', 'text', this._asset.category, (val) => {
      this._asset.category = val as string;
      this._asset.touch();
      this._emit();
    });
    metaSection.appendChild(catRow);
    ...
    fieldsTitleRow.innerHTML = `
      <span class="struct-editor-section-title">Fields</span>
      <button class="struct-editor-add-btn" title="Add Field">+ Add Field</button>
    `;
    ...
    fieldsTitleRow.querySelector('.struct-editor-add-btn')!.addEventListener('click', () => {
      const field = this._asset.addField('NewField', 'Float');
      this._emit();
    ...
    previewSection.innerHTML = `
      <div class="struct-editor-section-title">Default Value Preview</div>
  ...
  private _renderFields(container: HTMLElement): void {
    ...
    if (this._asset.fields.length === 0) {
      ...
      empty.textContent = 'No fields defined. Click "+ Add Field" to begin.';
    ...
    headerRow.innerHTML = `
      ...
      <span class="struct-field-col-tooltip">Tooltip</span>
  ...
  private _createFieldRow(field: StructureFieldDef, index: number, fieldsContainer: HTMLElement): HTMLElement {
    ...
    nameInput.addEventListener('change', () => {
      field.name = nameInput.value.trim() || 'Field';
      this._asset.touch();
      this._emit();
    });
  ...
  private _createDefaultValueInput(field: StructureFieldDef): HTMLElement {
    ...
    if (field.type === 'Float') {
      ...
      const enumAsset = this._manager.getEnum(enumId);
      const inp = document.createElement('select');
      inp.className = 'struct-field-select';
      if (enumAsset) {
