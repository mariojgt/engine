// ============================================================
//  ExportRuntime — Unified game runtime for exported builds
//
//  Contains ALL shared logic previously duplicated between
//  PCBuildTarget._generateGameRuntime() and
//  WebBuildTarget.generateWebRuntime().
//
//  Both build targets now generate a thin game_runtime.ts that
//  imports boot() from this module with a platform config.
// ============================================================
...
import { SaveLoadSystem } from '../engine/SaveLoadSystem';
import { ParticleSystemManager } from '../engine/ParticleSystem';
import { BehaviorTreeManager } from '../engine/BehaviorTreeManager';

// ── Expose THREE globally for blueprint new Function() closures ──
(globalThis as any).THREE = THREE;

// ── Boot configuration ──────────────────────────────────────

export interface ExportBootConfig {
  /** Name of the scene to load first */
  startScene: string;
  /** Target platform — controls renderer sizing, pixel ratio, polyfills, etc. */
  platform: 'pc' | 'web';
}

// ── Module-level state ──────────────────────────────────────

let _platform: 'pc' | 'web' = 'pc';
...
/** Global sprite sheet map — shared between setup2DScene() and loadAnimBP2DData() */
let spriteSheetMap = new Map<string, any>();
/** Monotonic ID counter for runtime-spawned 2D actors (starts high to avoid colliding with scene-loaded IDs) */
let _runtimeActorNextId = 90000;

// ── Lightweight TimerManager for blueprint timer nodes ──────
class RuntimeTimerManager {
  private _timers = new Map<number, { fn: () => void; interval: number; looping: boolean; paused: boolean; handle: number; remaining: number; elapsed: number }>();
  private _next = 1;

  setTimer(fn: () => void, interval: number, looping: boolean): number {
    const h = this._next++;
    this._timers.set(h, { fn, interval, looping, paused: false, handle: h, remaining: interval, elapsed: 0 });
    return h;
  }
  clearTimer(handle: number): void { this._timers.delete(handle); }
  pauseTimer(handle: number): void { const t = this._timers.get(handle); if (t) t.paused = true; }
  unpauseTimer(handle: number): void { const t = this._timers.get(handle); if (t) t.paused = false; }
  isTimerActive(handle: number): boolean { return this._timers.has(handle); }
  isTimerPaused(handle: number): boolean { return this._timers.get(handle)?.paused ?? false; }
  getTimerRemainingTime(handle: number): number { return this._timers.get(handle)?.remaining ?? 0; }
  getTimerElapsedTime(handle: number): number { return this._timers.get(handle)?.elapsed ?? 0; }
  ...
  clearAllTimers(): void { this._timers.clear(); }
}

let runtimeTimerManager: RuntimeTimerManager;
...
/** Get viewport dimensions — platform-dependent */
...
/** Universal quit handler — Tauri process exit or window.close() */
function quitGame(): void {
  console.log('[Runtime] Quit requested');
...
function installTauriPolyfill(): void {
...
// ── Expose-on-spawn variable overrides ──────────────────────
...
// ── Dynamic 2D actor spawning (from blueprint SpawnActor nodes) ──

function spawnRuntimeActor(
  classId: string,
  className: string,
  pos: any,
  rot: any,
  ...
  // In a 2D scene, skip the 3D Scene.spawnActorFromClass path entirely —
  // cooked actor JSONs don't have a nested `blueprintData` object and the
  // 3D path would crash.  Build the SpriteActor directly from asset data.
  if (is2DScene && root2D) {
    const goData = (engine as any).assetManager?.getAsset(classId);
    if (!goData) {
      console.warn('[Runtime 2D] spawnRuntimeActor: asset not found for', classId, className);
      return null;
    }
    try {
      ...
      if (sprComp?.spriteScale) (actor.spriteRenderer as any).spriteScale = { x: sprComp.spriteScale.x, y: sprComp.spriteScale.y };
      if (sprComp?.spriteOffset) (actor.spriteRenderer as any).spriteOffset = { x: sprComp.spriteOffset.x, y: sprComp.spriteOffset.y };

      actor.animBlueprintId = sprComp?.animBlueprint2dId ?? null;

      // Apply sprite sheet and default sprite (if not using an AnimBP)
      ...
      if (cm2dComp || goData.actorType === 'characterPawn2D') {
        const moveProps = {
          defaultWalkSpeed: 300, jumpZVelocity: 600, gravityScale: 1,
          airControl: 0.2, coyoteTime: 0.1, jumpBufferTime: 0.1, maxFallSpeed: 1000,
          ...movCfg,
        };
      ...
      if (goData.compiledCode) {
        ...
        if (overrides && Object.keys(overrides).length > 0) {
          (actor as any).__actorBlueprintCode = _applyExposeOnSpawnOverrides(
            (actor as any).__actorBlueprintCode, overrides,
          );
        }
      }
      ...
      // Reload AnimBP data so newly spawned actors get their sprite sheets
  ...
  // 3D path — delegate to the engine's Scene.spawnActorFromClass
...
async function setupSceneEnvironment(threeScene: THREE.Scene): Promise<void> {
  ...
  if (ppProps) {
    if (ppProps.toneMappingType === 'ACES') renderer.toneMapping = THREE.ACESFilmicToneMapping;
    else if (ppProps.toneMappingType === 'Reinhard') renderer.toneMapping = THREE.ReinhardToneMapping;
    else if (ppProps.toneMappingType === 'Cineon') renderer.toneMapping = THREE.CineonToneMapping;
    else if (ppProps.toneMappingType === 'Linear') renderer.toneMapping = THREE.LinearToneMapping;
    else if (ppProps.toneMappingType === 'AgX') renderer.toneMapping = THREE.AgXToneMapping;
    else if (ppProps.toneMappingType === 'Neutral') renderer.toneMapping = THREE.NeutralToneMapping;
...
async function initRuntimeAssets(projectMeta: any): Promise<void> {
  ...
  if (actorMap.size > 0) {
    ...
    // Build actorAssetManager adapter for blueprint class hierarchy nodes
    // Reads _inheritance data from cooked actor JSONs
    const actorAssetMgr = {
      getAsset(id: string) { return actorMap.get(id) ?? null; },
      getParentClass(id: string): string {
        const a = actorMap.get(id);
        return a?._inheritance?.parentClassId ?? '';
      },
      getChildClasses(id: string): string[] {
        const a = actorMap.get(id);
        return a?._inheritance?.childClassIds ?? [];
      },
      isChildOf(classId: string, parentId: string): boolean {
        ...
        while (current) {
          ...
          visited.add(current);
          const a = actorMap.get(current);
          const pid = a?._inheritance?.parentClassId;
          if (!pid) return false;
      ...
      getAncestryChain(id: string): string[] {
        ...
        while (current && !visited.has(current)) {
          ...
          const a = actorMap.get(current);
          current = a?._inheritance?.parentClassId ?? '';
        }
        return chain;
      },
    };
    (engine as any).actorAssetManager = actorAssetMgr;
  ...
  if (aiControllerMap.size > 0) {
    engine.aiAssetManager = {
      ...
      getAllTasks: () => [],
      getBlackboard: (_id: string) => null as any,
    } as any;
    console.log('[Runtime]   AI controller assets:', aiControllerMap.size);
  }

  // 2b. Full AI assets (BehaviorTrees, Blackboards, Tasks) from ai_assets.json
  try {
    const res = await fetch('/project-data/AI/ai_assets.json');
    if (res.ok) {
      ...
      for (const bt of (aiData.behaviorTrees ?? [])) btMap.set(bt.id, bt);
      for (const bb of (aiData.blackboards ?? [])) bbMap.set(bb.id, bb);
      for (const t of (aiData.tasks ?? [])) taskMap.set(t.id, t);

      // Create BehaviorTreeManager
      const btManager = new BehaviorTreeManager(
        (id: string) => btMap.get(id),
        (assetRef: string) => {
          const task = taskMap.get(assetRef);
          return task?.compiledCode ?? undefined;
        },
      );
      btManager.setEngine(engine);
      engine.behaviorTreeManager = btManager;

      // Expand aiAssetManager with blackboard access
      if (!engine.aiAssetManager) engine.aiAssetManager = {} as any;
      const prevGetBB = engine.aiAssetManager.getBlackboard;
      engine.aiAssetManager.getBlackboard = (id: string) => bbMap.get(id) ?? (prevGetBB ? prevGetBB(id) : null);
      engine.aiAssetManager.getBehaviorTree = (id: string) => btMap.get(id) ?? null;

      console.log('[Runtime]   AI full assets: BTs=' + btMap.size + ' BBs=' + bbMap.size + ' Tasks=' + taskMap.size);
    }
  } catch { /* no ai_assets.json */ }
        ...
        try {
          ...
          if (r.ok) {
            ...
            if (mapping.actionMappings || mapping.axisMappings) {
              engine.input.loadMappings(mapping.actionMappings ?? [], mapping.axisMappings ?? []);
...
function _buildCollisionShim(actor: SpriteActor): any {
  return {
    registerCallbacks(_goId: number) {
      ...
      const mapEvt = (evt: any) => ({
        otherActorName:    evt.otherName ?? evt.otherActor?.name ?? '',
        otherActorId:      evt.otherActor?.id ?? 0,
        selfComponentName: evt.selfComponentName ?? '',
      });
      actor.on('triggerBegin2D',   (e: any) => { const m = mapEvt(e); for (const cb of cbs.onBeginOverlap) cb(m); });
      actor.on('triggerEnd2D',     (e: any) => { const m = mapEvt(e); for (const cb of cbs.onEndOverlap)   cb(m); });
      actor.on('collisionBegin2D', (e: any) => { const m = mapEvt(e); for (const cb of cbs.onBeginOverlap) cb(m); });
      actor.on('collisionEnd2D',   (e: any) => { const m = mapEvt(e); for (const cb of cbs.onEndOverlap)   cb(m); });
      return cbs;
    },
    isOverlapping() { return false; },
    getOverlappingCount() { return 0; },
    getOverlappingIds() { return []; },
    teleportBody(_physics: any, _goId: number, pos: any) {
      // 2D: directly set actor position
      if (actor.group) { actor.group.position.set(pos.x ?? 0, pos.y ?? 0, pos.z ?? 0); }
    },
    setBodyEnabled(_physics: any, _goId: number, enabled: boolean) {
      // Enable/disable the actor's 2D rigid body via Rapier's setEnabled API
      const body = actor.physicsBody;
      if (!body?.rigidBody) return;
      try {
        body.rigidBody.setEnabled(enabled);
      } catch {
        // Older Rapier builds may not have setEnabled — fall back to sleep/wake
        try {
          if (enabled) body.rigidBody.wakeUp();
          else body.rigidBody.sleep();
        } catch { /* ignore */ }
      }
    },
    resizeSensor(_physics: any, _goId: number, _componentIndex: number, config: any) {
      // Resize the actor's collider(s) by removing old and adding new with updated dimensions
      const body = actor.physicsBody;
      if (!body?.rigidBody || !physics2D?.world) return;
      const dims = config?.dimensions;
      if (!dims) return;
      try {
        // Remove existing colliders
        for (const col of body.colliders) {
          try { physics2D.world.removeCollider(col, false); } catch { /* ignore */ }
        }
        body.colliders.length = 0;
        body.colliderNames.clear();

        // Re-create based on shape type
        const shape = config.shape || 'box';
        const isTrigger = config.collisionMode === 'trigger' || config.isTrigger;
        const name = config.name || '';
        if (shape === 'circle' || shape === 'sphere') {
          const r = dims.radius ?? Math.max(dims.width ?? 1, dims.height ?? 1) / 2;
          physics2D.addCircleCollider(body.rigidBody, r, { isTrigger, name });
        } else if (shape === 'capsule') {
          const halfH = (dims.height ?? 1) / 2;
          const r = (dims.width ?? 0.5) / 2;
          physics2D.addCapsuleCollider(body.rigidBody, halfH, r, { isTrigger, name });
        } else {
          // box (default)
          const halfW = (dims.width ?? 1) / 2;
          const halfH = (dims.height ?? 1) / 2;
          physics2D.addBoxCollider(body.rigidBody, halfW, halfH, { isTrigger, name });
        }
        // Refresh the body entry's colliders from the bodyMap (addXxxCollider pushes to it)
        const refreshed = physics2D.bodyMap.get(body.rigidBody.handle);
        if (refreshed) {
          body.colliders = refreshed.colliders;
          body.colliderNames = refreshed.colliderNames;
        }
      } catch (err) {
        console.warn('[CollisionShim] resizeSensor error:', err);
      }
    },
  };
}

function _buildSceneShim(): any {
  return {
    ...
    spawnActorFromClass: (classId: string, className: string, pos: any, rot: any, sc: any, owner: any, overrides: any) => {
      return spawnRuntimeActor(classId, className, pos, rot, sc, owner, overrides);
    },
    get engine() { return engine; },
    get threeScene() { return engine.scene.threeScene; },
  };
}

/** Augment the real engine with any dynamic properties blueprints expect */
let _engineAugmented = false;
function _augmentEngine(): void {
  if (_engineAugmented) return;
  _engineAugmented = true;
  const e = engine as any;

  // ── Timer manager (blueprint nodes use __engine.timerManager) ──
  if (!e.timerManager) e.timerManager = runtimeTimerManager;
  // Legacy alias
  if (!e.timers) e.timers = e.timerManager;

  // ── SaveLoad system (blueprint nodes use __engine.saveLoad) ──
  if (!e.saveLoad) {
    try {
      e.saveLoad = new SaveLoadSystem();
    } catch {
      // SaveLoadSystem may fail if Tauri unavailable — use memory-only stub
      e.saveLoad = {
        createSaveGameObject: () => ({ variables: {} as Record<string, any>, setVariable(k: string, v: any) { this.variables[k] = v; }, getString(k: string, d = '') { return String(this.variables[k] ?? d); }, getInt(k: string, d = 0) { return parseInt(this.variables[k] ?? d); }, getFloat(k: string, d = 0) { return parseFloat(this.variables[k] ?? d); }, getBool(k: string, d = false) { return Boolean(this.variables[k] ?? d); }, getVector(k: string) { return this.variables[k] ?? { x: 0, y: 0, z: 0 }; } }),
        saveGameToSlot: () => false,
        loadGameFromSlot: () => null,
        getFullSaveData: () => null,
        deleteSaveGameInSlot: () => false,
  ...
  // ── Particle manager ──
  if (!e.particleManager) e.particleManager = ParticleSystemManager.getInstance();

  // ── Pause state ──
  if (!('isPaused' in e)) e.isPaused = false;

  // ── Game mode / Game state stubs ──
  if (!e.gameMode) e.gameMode = null;
  if (!e.gameState) e.gameState = null;

  // ── Play canvas ──
  e._playCanvas = canvas;

  // ── Wire spawnActor on engine ──
  if (!e.spawnActor || e.spawnActor === Engine.prototype.spawnActor) {
    // Ensure our runtime spawn is the primary path
    const originalSpawn = e.spawnActor?.bind(e);
    e.spawnActor = (classId: string, className: string, pos: any, rot: any, sc: any, owner: any, overrides: any) => {
      const result = spawnRuntimeActor(classId, className, pos, rot, sc, owner, overrides);
      if (result) return result;
      // Fallback to engine's own spawnActor if runtime spawn didn't find the asset
      if (originalSpawn) return originalSpawn(classId, className, pos, rot, sc, owner, overrides);
      return null;
    };
  }
  ...
  // ── Scene2D manager shim (so 2D spawn/destroy paths work) ──
  if (!e.scene2DManager) {
    e.scene2DManager = {
      get isPlaying() { return true; },
      get spriteActors() { return spriteActors2D; },
      get physics2D() { return physics2D; },
      get debugDraw() { return null; },
      spawnActorFromClassId: (classId: string, pos: any, overrides?: any) => {
        return spawnRuntimeActor(classId, '', pos, { x: 0, y: 0, z: 0 }, { x: 1, y: 1, z: 1 }, null, overrides);
      },
      despawnSpriteActor2D: (actor: any) => {
        if (actor) (actor as any).__pendingDestroy = true;
      },
    };
  }

  // ── anim2d subsystem (2D AnimBP state machine queries from blueprint nodes) ──
  if (!e.anim2d) {
    e.anim2d = {
      /** Return the current state ID for an actor's AnimBP */
      getCurrentState(actor: any): string {
        const entry = actorAnimBPStates.get(actor);
        return entry?.currentStateId ?? '';
      },
      /** Read an AnimBP variable for a given actor */
      getVariable(actor: any, varName: string): any {
        return (actor as any).__animVars?.[varName] ?? null;
      },
      /** Force-transition the actor's AnimBP from fromState to toState */
      transitionState(actor: any, fromState: string, toState: string): void {
        const entry = actorAnimBPStates.get(actor);
        if (!entry) return;
        // Only transition if the actor is actually in fromState (or fromState is '*' / empty for unconditional)
        if (fromState && fromState !== '*' && entry.currentStateId !== fromState) return;
        const sm = entry.abp?.stateMachine;
        if (!sm) { entry.currentStateId = toState; return; }
        const targetState = sm.states?.find((s: any) => s.id === toState);
        if (!targetState) { entry.currentStateId = toState; return; }
        entry.currentStateId = toState;
        // Drive the animator to play the target state's animation
        const animator = (actor as any).animator;
        if (animator) {
          if (targetState.blendSpace1D) {
            applyBlendSpace(targetState, (actor as any).__animVars ?? {}, animator);
  ...
  // ── Debug drawing — forwarded to real engine methods ──
  // engine.drawDebugLine and engine.drawDebugPoint already exist on Engine class
}

function _getEngine(): any {
  _augmentEngine();
  return engine;
}
...
function runActorBlueprintScript(actor: SpriteActor, deltaTime: number): void {
  ...
  if (!ev) {
    ...
    if (!ok) console.warn('[Runtime 2D] Failed to compile actor blueprint for "' + actor.name + '".');
  }
  if (!ev.script) return;
  ...
  const ctx = {
    ...
    physics:        { collision: _buildCollisionShim(actor), world: null },
    scene:          _buildSceneShim(),
    uiManager:      (engine as any).uiManager ?? null,
    animInstance:    null,
    meshAssetManager: MeshAssetManager.getInstance() ?? null,
    loadMeshFromAsset: loadMeshFromAsset ?? null,
    buildThreeMaterialFromAsset: buildThreeMaterialFromAsset ?? null,
    engine:         _getEngine(),
...
function flushPendingDestroys(): void {
  for (let i = spriteActors2D.length - 1; i >= 0; i--) {
    ...
    if (!actor.__pendingDestroy) continue;
    ...
    try { actor.dispose(physics2D ?? undefined); } catch { /* ignore */ }
  }
}

// ── AnimBP 2D helpers ───────────────────────────────────────

function syncAnimBPVars(actor: SpriteActor, dt: number): void {
...
function setupControllerShims(): void {
  for (const actor of spriteActors2D) {
    ...
    if (actorAny.actorAssetId == null && actorAny.blueprintId) actorAny.actorAssetId = actorAny.blueprintId;
  }
}

// ── AI Controllers for 2D ───────────────────────────────────
...
async function loadAnimBP2DData(): Promise<void> {
  ...
  for (const actor of spriteActors2D) {
    ...
    if (entryState?.spriteAnimationName && actor.animator) {
      if (entryState.spriteSheetId && (actor as any)._spriteSheet?.assetId !== entryState.spriteSheetId) {
...
async function setup2DScene(sceneData: any): Promise<void> {
  ...
  camera2D.resize(vp.width, vp.height);
  ...
  console.log('[Runtime 2D]   Physics2DWorld initialised (gravity:', JSON.stringify(worldSettings.gravity ?? { x: 0, y: -980 }), 'ppu:', ppu, ')');
    ...
    if (ts.imagePath) {
      try {
        ...
        await new Promise<void>((resolve) => {
          img.onload = () => { ts.image = img; console.log('[Runtime 2D]   Tileset image loaded:', ts.assetName, img.naturalWidth + 'x' + img.naturalHeight); resolve(); };
  ...
  console.log('[Runtime 2D]   Tilemap renderer built —', tilemaps.length, 'tilemaps,', tilesets.length, 'tilesets');
  ...
  // Tilemap collision bodies
  {
    ...
    console.log('[Runtime 2D]   Tile collision built — bodies=' + stats.bodies + ', colliders=' + stats.colliders);
  }

  // ── Sprite sheet loading ──
  const spriteSheets: any[] = config2D.spriteSheets ?? [];
  spriteSheetMap = new Map<string, any>();
  ...
  for (const ss of spriteSheets) {
    if (ss.imagePath) {
      try {
        ...
        img.crossOrigin = 'anonymous';
        await new Promise<void>((resolve) => {
          img.onload = () => { ss.image = img; console.log('[Runtime 2D]   SpriteSheet image loaded:', ss.assetName, img.naturalWidth + 'x' + img.naturalHeight); resolve(); };
          img.onerror = () => { console.warn('[Runtime 2D]   SpriteSheet image failed:', ss.imagePath); resolve(); };
    ...
    for (const goData of sceneData.gameObjects) {
      try {
        ...
        if (goData.actorType === 'characterPawn2D' || goData.actorType === 'spriteActor') {
          ...
          // CharacterMovement2D
          if (cm2dComp || goData.actorType === 'characterPawn2D') {
            const moveProps = { ...defaultCharacterMovement2DProps(), ...movCfg };
            const cm2d = new CharacterMovement2D(moveProps);
            cm2d.attach(actor);
            actor.characterMovement2D = cm2d;
            actor.setComponent('CharacterMovement2D', cm2d);
          ...
          if (cam2dComp && camera2D) {
            ...
            if (camConfig.defaultZoom) camera2D.setZoom(camConfig.defaultZoom);
            console.log('[Runtime 2D]   Camera2D follow:', goData.name, '(zoom:', camConfig.defaultZoom ?? 'default', ')');
          }

          // Actor blueprint code
          if (goData.compiledCode) {
            (actor as any).__actorBlueprintCode = goData.compiledCode;
          }

          spriteActors2D.push(actor);
          console.log('[Runtime 2D]   + SpriteActor:', goData.name, '(', goData.actorType, ') physics:', bodyType ?? 'none', 'pos:', spawnPos.x.toFixed(2) + ',' + spawnPos.y.toFixed(2));
        } else {
          console.log('[Runtime 2D]   + GO:', goData.name, '(', goData.actorType ?? 'default', ')');
        }
      } catch (err) {
...
async function loadSceneByName(sceneName: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) { console.error('[Runtime] Scene not found:', sceneName); return; }
    ...
    for (const [actor, entry] of actorBlueprintScripts.entries()) {
      if (entry.script && entry.started) {
        try { entry.script.onDestroy({ gameObject: actor as any, deltaTime: 0, elapsedTime: entry.elapsed } as any); } catch { /* */ }
      }
    }
    actorBlueprintScripts.clear();
    for (const [, entry] of actorAnimBPScripts.entries()) {
      if (entry.script && entry.started) {
        try { entry.script.onDestroy({} as any); } catch { /* */ }
      }
    }
    actorAnimBPScripts.clear();
    actorAnimBPStates.clear();
    for (const ctrl of aiControllers2D) { try { ctrl.destroy?.(); } catch { /* */ } }
    aiControllers2D = [];
    ...
    if (isLikely2DSceneData(sceneData) && sceneData.scene2DConfig) {
      ...
      console.log('[Runtime] 2D Scene loaded:', sceneName);
      return;
    }
    ...
    if (sceneData.gameObjects) {
      for (const goData of sceneData.gameObjects) {
        try {
          ...
          // Custom imported mesh instances
          if (goData.customMeshAssetId) {
            ...
            if (meshAsset) {
              try {
                ...
                if (goData.rotation) meshGo.mesh.rotation.set(goData.rotation.x, goData.rotation.y, goData.rotation.z);
                if (goData.scale) meshGo.mesh.scale.set(goData.scale.x, goData.scale.y, goData.scale.z);
                if (typeof goData.hasPhysics === 'boolean') meshGo.hasPhysics = goData.hasPhysics;
                if (goData.physicsConfig) meshGo.physicsConfig = structuredClone(goData.physicsConfig);
                if (goData.materialOverrides?.length > 0) {
          ...
          if (goData.rotation) go.mesh.rotation.set(goData.rotation.x, goData.rotation.y, goData.rotation.z);
          if (goData.scale) go.mesh.scale.set(goData.scale.x, goData.scale.y, goData.scale.z);
    ...
    // 5b. Apply deferred material overrides
    ...
    // 6. Restart physics
    try { engine.physics.play(engine.scene); console.log('[Runtime]   Physics started'); }
    catch (err) { console.error('[Runtime]   Physics play failed:', err); }
    ...
    console.log('[Runtime] Scene loaded:', sceneName);
  } catch (err) {
...
const runtimeProjectManager = {
  async loadSceneRuntime(sceneName: string): Promise<boolean> {
    try { await loadSceneByName(sceneName); return true; }
    catch (err) { console.error('[Runtime] loadSceneRuntime failed:', err); return false; }
  },
...
function startGameLoop(): void {
  ...
  function loop(now: number) {
    ...
    // Tick timers + debug drawing before main update
    runtimeTimerManager.tick(dt);
    engine._tickDebugDraw(dt);
    ...
    if (is2DScene && camera2D) {
      ...
      for (const ctrl of aiControllers2D) { try { ctrl.update(dt); } catch { /* */ } }
      for (const actor of spriteActors2D) {
        actor.update(dt);
      ...
      if (physics2D) { try { physics2D.processEvents(); } catch { /* */ } }
      flushPendingDestroys();

      const s = vp();
      camera2D.resize(s.width, s.height);
      camera2D.update(dt);
      ...
      if (cam) {
        ...
        fallbackCamera.aspect = s.width / s.height;
...
export async function boot(config: ExportBootConfig): Promise<void> {
  _platform = config.platform;

  // Install Tauri polyfill for web platform
  if (_platform === 'web') {
  ...
  // Create renderer with platform-specific settings
  renderer = new THREE.WebGLRenderer({ canvas, antialias: true });
  renderer.setPixelRatio(
    _platform === 'web' ? Math.min(window.devicePixelRatio, 2) : window.devicePixelRatio,
  );
  ...
  const resize = () => {
    const vp = getViewportSize();
    renderer.setSize(vp.width, vp.height, _platform === 'web' ? false : undefined);
  };
  resize();
  ...
  engine = new Engine();
  runtimeTimerManager = new RuntimeTimerManager();
  ...
  // Wire projectManager, quit handler, and augment engine with runtime subsystems
  engine.projectManager = runtimeProjectManager;
  (engine as any).quit = quitGame;
  _augmentEngine();
  ...
  let projectMeta: any = {};
  // PC uses /project-data/project.json, Web uses /project-data/Config/project.json
  const metaPaths = _platform === 'pc'
    ? ['/project-data/project.json', '/project-data/Config/project.json']
    : ['/project-data/Config/project.json', '/project-data/project.json'];
  for (const path of metaPaths) {
    try {
      const pmRes = await fetch(path);
      if (pmRes.ok) { projectMeta = await pmRes.json(); break; }
    } catch { /* try next */ }
  }

  // Load all runtime assets
  setProgress(17, 'Loading runtime assets...');
  await initRuntimeAssets(projectMeta);

  // Save default 3D renderer settings
  ...
  // Fallback camera
  const vp = getViewportSize();
  fallbackCamera = new THREE.PerspectiveCamera(60, vp.width / vp.height, 0.1, 1000);
  fallbackCamera.position.set(5, 4, 5);
  fallbackCamera.lookAt(0, 0, 0);

  // Determine start scene
  const startScene = config.startScene || projectMeta.activeScene || 'DefaultScene';
  ...
  // Hide loading overlay (platform-appropriate animation)
  if (loadingOverlay) {
    if (_platform === 'web') {
      setTimeout(() => {
